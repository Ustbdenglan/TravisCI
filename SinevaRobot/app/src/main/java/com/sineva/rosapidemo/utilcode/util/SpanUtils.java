...
//            }
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(SinevaUtils.getApp().getResources(), b);
            mDrawable.setBounds(
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                try {
                    InputStream is =
                            SinevaUtils.getApp().getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(SinevaUtils.getApp().getResources(), bitmap);
                    drawable.setBounds(
                ...
                try {
                    drawable = ContextCompat.getDrawable(SinevaUtils.getApp(), mResourceId);
                    drawable.setBounds(
                            0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight()
                    );
                } catch (Exception e) {
